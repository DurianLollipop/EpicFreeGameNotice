...
> Epic 喜加一通知机器人：基于 GitHub Actions 自动检测 Epic Games Store 免费游戏，并通过 Telegram Bot 或 Bark 推送图文通知。
...
这是一个零服务器成本的自动化脚本。你只需要 Fork 仓库，并在 GitHub Secrets 中配置 Telegram Bot 或 Bark 信息，就可以每天自动接收 Epic 免费游戏通知。
...
| 图文通知 | Telegram 支持图文预览，Bark 支持封面图标、正文和跳转链接 |
...
你可以只配置 Telegram，只配置 Bark，也可以两个都配置。
...
```text
BARK_URL=https://你的Bark服务地址/你的Key
```
...
Bark 会把游戏标题和简介作为通知文字，把游戏封面图作为通知图标，并把领取链接放到通知点击跳转里。由于 Bark 的通知展示由 iOS 控制，它不像 Telegram 那样在消息正文里展示大图。
...
Epic Games Free Notifier is a GitHub Actions based automation script that checks Epic Games Store free games every day and sends rich Telegram or Bark notifications.

No server is required. Fork the repository, configure Telegram or Bark secrets, and run the workflow.
...
| Rich notification | Telegram supports rich preview; Bark supports icon, text, group, and click-through URL |
